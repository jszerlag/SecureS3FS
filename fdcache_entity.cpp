...
//set /home/jmboy/password to whatever password file you would like ( < 256 bytes )
#define PASSWORD_FILE "/home/jmboy/password"

static int decrypt_file_fd(int fd)
{
    printf("Inside NEW Decrypt Function\n");
    ...
    lseek(fd, 8, SEEK_SET); // seek to salt
    ...
    FILE *fp = fopen(PASSWORD_FILE, "r");   // was having trouble reading using descriptor
    if (!fp) {
        printf("Error opening password file, using default password\n");
        strcpy(password, "password");
    } 
    else {
        if (!fgets(password, sizeof(password), fp)) {
            printf("Error reading password file, using default password\n");
            strcpy(password, "password");
        } 
        else {
    ...
    if(!(ctx = EVP_CIPHER_CTX_new())) {
        printf("Error inside EVP_CIPHER_CTX_new()\n");
        return 1;
    }
    ...
    //track where we are reading and writing from
    off_t readPos = 16;
    off_t writePos = 0;

    while ((numRead = read(fd, buf, BUFF_SIZE)) > 0) {
        readPos += numRead;

        if(1 != EVP_DecryptUpdate(ctx, outbuf, &len, buf, numRead)) {
            printf("EVP_DecryptUpdate() returned error or partial write occurred\n");
            return 1;
        }
    ...
    if(1 != EVP_DecryptFinal_ex(ctx, outbuf, &len)){
        printf("Error inside EVP_DecryptFinal_ex()\n");
        return 1;
    }

    lseek(fd, writePos, SEEK_SET);   // seek back to writing location

    if (write(fd, outbuf, len) != len){
        printf("Error inside final write()\n");
        return 1;
    }

    writePos += len;

    ftruncate(fd, writePos); // make sure extra bytes at end of file are removed
...
static int encrypt_file_fd(int fd)
{
    printf("inside NEW encrypt function\n");

    int BUFF_SIZE = 4096;
    unsigned char buf[BUFF_SIZE];
    unsigned char cipherbuf[BUFF_SIZE];

    unsigned char key[32];
    unsigned char iv[16];
    unsigned char salt[8];

    RAND_bytes(salt, 8); // create random salt
    ...
    /* track where we are reading and writing from 
    - Will want to read extra buffer ahead of write for encryption to avoid overwriting original file bytes */

    off_t readPos = 0;
    off_t writePos = 0;

    // read into buffer
    if ((numRead = read(fd, buf, BUFF_SIZE)) < 0) { 
        return 1;
    }
    readPos += numRead;
    ...
    if(1 != EVP_EncryptUpdate(ctx, cipherbuf, &len, buf, numRead)) {
        printf("first EVP_EncryptUpdate() returned error or partial write occurred\n");
        return 1;
    }

    while ((numRead = read(fd, buf, BUFF_SIZE)) > 0) {  // read next chunk(s) before overwriting with write - now a chunk ahead
        readPos += numRead;

        lseek(fd, writePos, SEEK_SET);

        if (writePos == 0) {    // if at start of file, write Salted__(salt)
            write(fd, "Salted__", 8);
            write(fd, salt, 8);
            writePos += 16;
        }

        if (write(fd, cipherbuf, len) != len) {
            printf("write() returned error or partial write occurred\n");
            return 1;
        }
    ...
    lseek(fd, writePos, SEEK_SET);
    // extra write to ensure all data is written
    if (writePos == 0) {
        write(fd, "Salted__", 8);
        write(fd, salt, 8);
        writePos += 16;
    ...
    if(1 != EVP_EncryptFinal_ex(ctx, cipherbuf, &len)){
        printf("Error inside EVP_EncryptFinal_ex()\n");
        return 1;
    }
...
int FdEntity::Load(off_t start, off_t size, bool is_modified_flag)
{
    ...
    if(0 < pagelist.GetUnloadedPages(unloaded_list, start, size)){
        for(auto iter = unloaded_list.cbegin(); iter != unloaded_list.cend(); ++iter){
          ...
          result = decrypt_file_fd(physical_fd);
          if(0 != result){
...
int FdEntity::RowFlushHasLock(int fd, const char* tpath, bool force_sync)
{
    ...
    result = encrypt_file_fd(physical_fd);
    if(0 != result){
        printf("Failed to encrypt file data\n");
        return result;
    }
